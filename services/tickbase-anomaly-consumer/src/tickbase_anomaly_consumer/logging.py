...
KEY_VALUE_RE = re.compile(
    r"\b((?:authorization|token|secret|signature|api[_-]?key|password|oauth)"
    r"\s*[=:]\s*)[^\s&,;]+",
    re.IGNORECASE,
)
...
def _is_sensitive_key(key: str) -> bool:
    ...
    return any(part in normalized.split("_") for part in SENSITIVE_KEY_PARTS) or any(
        part in normalized
        for part in ("api_key", "access_token", "bearer_token", "database_url")
    )
...
def configure_logging(level: str) -> None:
    ...
    handler.setFormatter(JsonLogFormatter(os.environ.get("SERVICE_NAME", "tickbase-anomaly-consumer")))
