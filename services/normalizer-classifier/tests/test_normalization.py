    ...
    return RoutingContext(
        domains=(
            DomainOption(
                ...
                threshold=30,
                sort_order=10,
                prompt_body="Geopolitics lens",
                keywords=(
                    DomainKeyword("iran", "iran", 30, "word"),
                    DomainKeyword("iran", "nuclear", 30, "word"),
                    DomainKeyword("military", "irgc", 30, "word"),
                    DomainKeyword("sanctions", "sanctions", 30, "word"),
                ),
            ),
        )
    )
...
def test_normalize_item_prefilter_passes_high_value_keywords() -> None:
    normalized = normalize_item(
        raw_item("Iran nuclear deal", "IRGC warns about sanctions"),
        source(),
        routing_context(),
    )

    assert normalized.prefilter_passed is True
    assert "geopolitics:iran" in normalized.matched_keywords
    assert "geopolitics:military" in normalized.matched_keywords
    assert normalized.keyword_score >= 60


def test_normalize_item_skips_empty_text() -> None:
    normalized = normalize_item(raw_item(None, None), source(), routing_context())
