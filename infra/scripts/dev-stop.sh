...
stop_pid() {
  ...
  if [[ "$command_line" != *"telegram-collector"* && "$command_line" != *"rss-collector"* && "$command_line" != *"normalizer-classifier"* && "$command_line" != *"alert-dispatcher"* && "$command_line" != *"telegram-channel-publisher"* && "$command_line" != *"tickbase-anomaly-consumer"* && "$command_line" != *"x-publisher"* && "$command_line" != *"dashboard-api"* && "$command_line" != *"npm run dev"* && "$command_line" != *"vite"* ]]; then
...
stop_matching_processes "tickbase-anomaly-consumer" "tickbase-anomaly-consumer run"
