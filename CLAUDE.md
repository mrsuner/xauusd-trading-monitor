...
`event-router` is the sole service that decides exit routes. It polls **unrouted** `events` (`worker.py` → `db.unrouted_event_ids`), computes a neutral `route_score` (`policy.py` `route_score()`), runs per-route policy, and writes: `alerts` for private Telegram / Pushover delivery, `public_outbox` for public publishers, and an `event_route_decisions` audit row per (event, route) — queued or skipped with a reason (migration `0011`, unique on `(event_id, route_key)`). `alert-dispatcher` now only claims pending / retry `alerts` rows and sends them (`worker.py` is claim-and-deliver; its legacy `policy.py` + `create_alert_decisions` scan path survives but is gated behind `ENABLE_POLLING_FALLBACK`, default **false**). `telegram-channel-publisher` only claims public Telegram rows from `public_outbox`.

Route keys + enable defaults (`event-router/settings.py`): `private.telegram`, `private.pushover`, `public.telegram_channel` are **on**; `public.website`, `public.x` are **off** (V2). Each route has an env-tunable score threshold (e.g. `PRIVATE_TELEGRAM_ROUTE_SCORE_THRESHOLD=45`, S-events `=35`; pushover `=85`; public telegram `=65`). Backfill events (created before service start) are skipped for non-telegram routes per `EVENT_ROUTER_BACKFILL_MODE` (default `telegram_only`). Source rate-limit / cooldown is enforced via `db.alert_channel_stats`. Deterministic message text (no AI) is built in `event-router/message.py`; publishers do final platform packaging only. Spec detail in `docs/services/event-router.md`.
